...
def fchoice_algorithm(board, cells, cage_sum, call):
    ...
    track_cage = 0
    if call == 'main':
        cages_length = len(cages)
    elif call == 'get_cage_sum':
    ...
    while True:
        ...
        for perm in permutations(numbers, len(cells)):
            if sum(perm) == cage_sum:
                ...
                row_errors, col_errors, duplicates = check_cage(board, call)
...
def main():
    ...
    row_errors, col_errors, duplicates = check_cage(board, 'main')
    cages_length = len(cages) - 1
    ch_cage_sum, ch_cells = cages[cages_length]
